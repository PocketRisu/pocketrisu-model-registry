...
| SQUARE1 (arca.live) | `GET https://inference.square1.dev/v1/models` | Bearer `sq-arca-…` | Full spec — `owned_by`, `supports_{tools,reasoning,images,embeddings}`, `max_input_tokens`, `max_output_tokens`, `rate_limit_rpm`. **The gateway host is not the dashboard domain**: the dashboard is `dash.square1.dev` (whose `/api/*` is a separate account API); the LLM gateway is `inference.square1.dev` (moved off the legacy `api.wellspring.encrypt.gay`, which now times out — 2026-09-26). Official docs + OpenAPI at `https://docs.square1.dev/llms.txt` / `/openapi.yaml`; per-model `reasoning_effort` values come back in the 400 `invalid_reasoning_effort` message (probe with a bogus value — rejected requests burn no quota/RPM). `GET /v1/usage` and `/v1/me` report quota and account. Key-gated, so this listing needs a member key — the public `https://dash.square1.dev/api/stats/fleet` gives IDs, health, `max_input_tokens` and a `disabled[]` list without one (`/api/stats/models` also lists internal models absent from `/v1/models`). |
...
| 2026-09-03 (follow-up) | Fixed the five stale pins the morning pass had recorded. Re-confirmed each against its live listing before touching it: Vercel `spacexai/grok-4.5` (500000/500000, unchanged, so limits untouched); NanoGPT `z-ai/glm-5.1` / `z-ai/glm-5.2` (bare ids kept — NanoGPT also exposes `:thinking` variants, and the previous pins were bare); Lightning AI now lists only `lightning-ai/nemotron-3-ultra-550b-a55b` in the Nemotron family; Cerebras — `api.cerebras.ai/public/v1/models` (2 models), the docs overview page (same 2), and `GET /public/v1/models/zai-glm-4.7` → **404 "not found or not publicly available"** (control `gemma-4-31b` → 200). | **3 renames, 2 deprecations**, all `version` +1 / `updatedAt` bumped: `vercel:grok-45` → `spacexai/grok-4.5`; `nanogpt:glm-51` → `z-ai/glm-5.1`; `nanogpt:glm-52` → `z-ai/glm-5.2`; `lightning-ai:nemotron-3-nano-omni` → `deprecated` (note points at `lightning-ai:nemotron-3-ultra`); `cerebras:zai-glm-47` → `deprecated` (Cerebras now serves only `gpt-oss-120b` + `gemma-4-31b`; `cerebras:openai-compatible` remains as the catch-all). Every non-deprecated pin on the seven keyless catalogs now resolves. Profile count unchanged (255). **Watch:** Vercel's `xai/` → `spacexai/` vendor-prefix rename will hit any future Grok pin; Grok 4.6 (`spacexai/grok-4.6`, 500000/500000) is live on Vercel but still not pinned (deferred since 2026-08-14). |
| 2026-09-23 | Targeted refresh driven by the **Claude Opus 5.5** and **GPT-6 Sol / Luna** launches (2026-09-22). Tier 0 sweep: OpenRouter (454 models) + the keyless listings of Cerebras / NeuralWatt / Vercel / Lightning AI / NanoGPT / Ollama Cloud / LLM Gateway. IDs confirmed against first-party SDK literals (openai-python `chat_model.py` lists `gpt-6-sol`/`gpt-6-luna`; anthropic-sdk-python `model_param.py` lists `claude-opus-5-5`/`claude-fable-5-1`) and the platform.claude.com models overview (API + Bedrock IDs). Specs: Opus 5.5 1M/128k, **adaptive thinking always on** (`thinking.type` disabled/enabled → 400), forced `tool_choice` any/tool rejected, default effort `medium`; Fable 5.1 (GA 2026-09-01) 1M/128k, same wire as Fable 5; GPT-6 Sol/Luna 1050000/128000, reasoning optional (`none`…`max`). **Fable 5.1 had been missed by the 2026-09-03 pass.** | Registry **260 → 280 profiles**, 22 bases. Added `anthropic:opus-55` / `anthropic:fable-51` (clones of `anthropic:fable-5` — always-on thinking; no `tool_choice` field so the forced-tool break does not apply), `openai:gpt-6-sol` / `openai:gpt-6-luna` (clones of `gpt-56-*`, `reasoning_effort` gains `max`), and openrouter / vercel / llmgateway / nanogpt mirrors of all four (every pin resolves on its live listing; Lightning AI lists none yet). `anthropic-compatible` and `bedrock-anthropic` base comboboxes gain both Claude IDs (build auto-bumped 3 inheriting profiles). *-pro GPT-6 variants excluded (Responses-only, as with Astra Pro). Opus 5 / Fable 5 / GPT-5.6 stay `current`. **Shipped develop→main without the in-app live-request gate** per user direction. **Deferred:** Grok 4.7 (supersedes the still-deferred 4.6; Vercel `spacexai/grok-4.7`), GLM 5.3 / 5.3 Flash, DeepSeek V4.1 Flash (DeepSeek direct now recommends `deepseek-flash`; legacy `deepseek-v4-flash` is still accepted and served by V4.1, so existing pins keep working), Qwen3.8 family, MiMo v2.6, Muse Spark 1.3. Drift noted, not fixed: existing GPT-5.6 profiles lack `max` in `reasoning_effort`. Bundle sync into NodeOnly still deferred to the next app release. |
| 2026-09-23 (follow-up) | Took four of the morning's deferred families per user direction. Limits read from each gateway's live listing (OpenRouter, Vercel, LLM Gateway, NeuralWatt); Ollama Cloud tags confirmed on the library pages (`glm-5.3:cloud`, `deepseek-v4.1-flash:cloud`); DeepSeek direct from api-docs.deepseek.com pricing + thinking-mode pages. | Registry **280 → 303 profiles**. **Grok 4.7**: openrouter / nanogpt / vercel (`spacexai/grok-4.7`) / llmgateway (`grok-4-7`); Grok 4.6 skipped as superseded. **GLM 5.3**: openrouter / nanogpt / vercel / llmgateway / neuralwatt / ollama-cloud (square1 skipped, key-gated listing; GLM 5.3 Flash not taken). **DeepSeek V4.1 Flash**: openrouter / nanogpt / vercel / ollama-cloud mirrors, plus new direct `deepseek:v41-flash` and `deepseek:v41-flash-thinking` pinned to DeepSeek's new recommended id `deepseek-flash` (1M / 384K; `reasoning_effort` low/high/max per the thinking-mode page). Direct `deepseek:v4-flash` / `v4-flash-thinking` → `outdated`: the V4 Flash model is retired and its legacy id is now served by V4.1. Gateway V4 Flash mirrors stay `current` (still listed). **Qwen3.8 Max** (openrouter / vercel / nanogpt pinned to the dated `qwen3.8-max-0902` — OpenRouter lists only that slug, and Vercel's bare `alibaba/qwen3.8-max` reports a 262144 window vs 991000 for the dated one; llmgateway `qwen3.8-max`) and **Qwen3.8 Flash** (openrouter / vercel / nanogpt, following the Qwen3.7 Plus mirror set). `ollama-cloud` / `llmgateway` base comboboxes deliberately left unchanged so no inheriting preset is nudged. Shipped develop→main without the in-app live-request gate per user direction. |
| 2026-09-26 | SQUARE1 (챈섭) API refresh. Sources: docs.square1.dev (base-urls, openapi.yaml, changelog), keyed `GET /v1/models`, `invalid_reasoning_effort` probes per model, public `/api/stats/fleet`. | Registry **303 → 308 profiles**. **Gateway host moved** `api.wellspring.encrypt.gay` → `inference.square1.dev` (old host times out): endpoint repointed on both existing profiles. Base provider: dropped `frequency_penalty` / `presence_penalty` / `seed` (not in the documented request contract; a live request confirmed the gateway silently ignores them, so installed presets carrying them keep working). `square1:glm-52` limits 49152 → 300000 in / 32768 out. `square1:deepseek-v4-flash-0731` → `deprecated` (disabled on the gateway). **New** (all tools + reasoning, `reasoning_effort` none/low/high/max matching the base enum): `glm-53` (800K/32K, 10 RPM), `glm-53-flash` (500K/32K, 10), `deepseek-v41-flash` (1M/128K, 10), `glm-52-code` (350K/32K, 20), `qwen38-27b` (250K/16K, 10). Only one live request (glm-5.3-flash on the new host) was sent; the new profiles were not exercised in-app. |
