...
jq -r 'select(.modelId != "") | .modelId' profiles/openrouter/*.json | sort > /tmp/ours.txt
comm -23 /tmp/ours.txt /tmp/live.txt   # anything printed is a profile that will fail
```

A miss is expected only for profiles already marked `deprecated`. Anything `current` that misses is either a typo or provider drift — both are user-visible failures.
