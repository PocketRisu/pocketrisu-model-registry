...
| `ModelProfile` (`profiles/<baseId>/<profileKey>.json`) | concrete profile a user picks (e.g. `openai:gpt-55`, `openrouter:openai-compatible`). Pins endpoint, auth wiring, `modelId`, profile status, and may extend the base's schema/UI. |
...
9. `profileStatus` ∈ {`current`, `outdated`, `deprecated`}.
...
| `ModelProfile` (`profiles/<baseId>/<profileKey>.json`) | 사용자가 직접 고르는 단위 (예: `openai:gpt-55`). endpoint, auth 와이어링, `modelId`, 프로필 상태 고정. base의 schema/UI를 확장 가능. |
